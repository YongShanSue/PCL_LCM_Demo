...
void TagDetector::extractFloatImages(
		const cv::Mat& image, std::vector<FloatImage> &float_images){
...
#pragma omp parallel for
	...
	//cout << "step 2 ok" << endl;
	float_images.push_back(fimOrig);
	float_images.push_back(fimSeg);
	float_images.push_back(fimTheta);
	float_images.push_back(fimMag);

}

// Step 3-6
void TagDetector::extractSegments(
	...
	FloatImage fimSeg = float_images[1];
	FloatImage fimTheta =  float_images[2];
	FloatImage fimMag =  float_images[3];
	...
	for (clustersItr = clusters.begin(); clustersItr != clusters.end(); clustersItr++) {
		...
		// filter by mask
		cv::Point2f pt1 = cv::Point2f(gseg.getP0().first, gseg.getP0().second);
		cv::Point2f pt2 = cv::Point2f(gseg.getP1().first, gseg.getP1().second);
		if(im_mask.at<uchar>(pt1) == 0 || im_mask.at<uchar>(pt2) == 0){
			continue;
		}
